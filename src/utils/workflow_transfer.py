...
def submission_snapshot(submission, active_application=None, sme_application=None):
    ...
    for candidate in (active_application, sme_application):
        if isinstance(candidate, dict) and candidate.get("application_id") == application_id:
            return dict(candidate)
    return {}


def submitted_intake_rows(submissions, lifecycles, active_application=None, sme_application=None):
    rows = []
        ...
        rows.append(
            {
                ...
                "Company": submission.get("company_name") or snapshot.get("company_name", "Applicant"),
                "Submitted": submission.get("timestamp") or submission.get("submitted_at", ""),
                "Status": lifecycle.get("status") or submission.get("status", "Submitted to lender review"),
...
def find_submitted_application(submissions, application_id, active_application=None, sme_application=None):
