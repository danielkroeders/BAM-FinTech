...
def application_table(frame, columns, aliases=None):
    ...
    for column in ["requested_amount", "existing_debt", "annual_revenue", "free_cash_flow", "final_probability"]:
    ...
    if "final_probability" in display:
        display["final_probability"] = display["final_probability"].apply(format_percent)
    if "expected_runway_months" in display:
        display["expected_runway_months"] = display["expected_runway_months"].apply(format_months)
    if "document_completeness_score" in display:
        display["document_completeness_score"] = display["document_completeness_score"].apply(format_score)
    if "missing_documents" in display:
        display["missing_documents"] = display["missing_documents"].apply(format_integer)
