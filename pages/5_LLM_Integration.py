...
st.caption(
    "Connect deterministic, hosted, or local-model second-review explanations to the latest scored loan request."
)
    ...
    st.session_state.llm_review_history.append(
        {
            ...
            "AI review score": (
                f"{ai_review_score}/100" if ai_review_score is not None else "N/A"
            ),
...
if not application or not prediction:
    st.info(
        "No application has been scored yet. Use Personal Workspace to create the first decision."
    )
    safe_page_link(
    ...
    with st.container(border=True):
        ...
        st.caption(
            "This model creates the baseline score and grade before any optional hosted or local LLM review."
        )

    rf_cols = st.columns(4)
    rf_cols[0].metric(
        "Application risk score", format_percent(prediction["fraud_probability"])
    )
    ...
    metric_cols[1].metric(
        "Model precision", format_score(metrics.get("precision", 0), 3)
    )
    metric_cols[2].metric(
        "Balanced accuracy", format_score(metrics.get("balanced_accuracy", 0), 3)
    )
    metric_cols[3].metric(
        "Precision top 10%", format_score(metrics.get("precision_at_10pct", 0), 3)
    )
    ...
    with st.form("llm_explanation_form"):
        ...
        if provider == "OpenAI API":
            openai_model = st.selectbox(
                ...
                index=(
                    ["gpt-4.1-mini", "gpt-4.1", "gpt-4o-mini"].index(
                        st.session_state.explanation_model
                    )
                    if st.session_state.explanation_model
                    in ["gpt-4.1-mini", "gpt-4.1", "gpt-4o-mini"]
                    else 0
                ),
            )
            ...
            local_api_key = st.text_input(
                "Local API token", value=local_api_key, type="password"
            )
            st.caption(
                "Enter the server root or `/v1` base URL. The app calls `/v1/chat/completions` when you click Run."
            )
            st.caption(
        ...
        run_explanation = st.form_submit_button(
            "Generate Internal + SME Reports", width="stretch"
        )
    ...
    if save_local_settings:
        try:
            ...
            st.success(
                f"Local model settings saved outside the repository: {saved_profile_path}"
            )
    ...
    if run_explanation:
        ...
        st.session_state.llm_chat_error = (
            " ".join(generated["errors"]) if generated["errors"] else None
        )
        st.session_state.llm_chat_signature = signature
    ...
    saved_package = st.session_state.llm_evaluation_packages.get(
        application.get("application_id", "Session")
    )
    current_package = (
        saved_package
        if saved_package and saved_package.get("signature") == signature
        else None
    )
    ...
    status_cols[2].metric(
        "Last run", st.session_state.get("llm_chat_last_run", "Not run yet")
    )
    ...
    if saved_package and not current_package:
        st.warning(
            "The saved evaluation package is stale because this application has been rescored. Generate a new package before publication."
        )

    internal_tab, sme_tab = st.tabs(
        ["Internal lender report", "SME-facing report draft"]
    )
    with internal_tab:
        st.caption(
            "Private decision-support output. This report is never published to the SME portal."
        )
        st.info(explanation)
    with sme_tab:
        if current_package:
            ...
            st.info(
                "Generate the evaluation package to create and attach an SME-facing report draft."
            )
    ...
    try:
        shap_table, baseline_probability, predicted_probability = shap_driver_table(
            st.session_state.model_bundle, application
        )
        ...
        summary_cols[1].metric(
            "Application Risk", format_percent(predicted_probability)
        )
        summary_cols[2].metric(
            "Largest Driver", top_drivers.iloc[0]["driver"].replace("_", " ").title()
        )
        ...
        display_table["SHAP contribution"] = display_table["SHAP contribution"].apply(
        ...
        st.warning(
            "Install the `shap` dependency from requirements.txt to view SHAP driver analysis."
        )
